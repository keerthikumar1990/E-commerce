                ...
                <table class="table table-striped table-bordered" style="width: 100%">
                  ...
                  <tbody>
                    <tr ng-repeat="data in filtered = (list | filter:search | orderBy : predicate :reverse) | startFrom:(currentPage-1)*entryLimit | limitTo:entryLimit ">
                      ...
                      <td><input type="number" class="form-control" id="bamt_{{data.post_id}}"  style="width: 70px; display: inline-block" required/><button class="btn btn-primary" id="placebid_{{data.post_id}}" style="margin-left: 10px">Place Bid</button></td>
                    </tr>
                  </tbody>
                </table>
                </form>
	    	...
	    	$.ajax({
				...
				success : function(responseText) {
					if(responseText == 'inserted'){
						$('#bidalert').removeClass('alert-danger');
						$('#bidalert').html("");
						$('#bidalert').addClass('alert-success');
						$('#bidalert').html("<strong>Success!</strong> Your bid has been placed successfully.");
						$('#bidalert').css("display","block");		
					}else if(responseText == 'updated'){
						$('#bidalert').removeClass('alert-danger');
						$('#bidalert').html("");
						$('#bidalert').addClass('alert-success');
						$('#bidalert').html("<strong>Success!</strong> You have already placed a bid for this post. Your bid amount has been updated successfully.");
						$('#bidalert').css("display","block");		
					}
					else{
						$('#bidalert').removeClass('alert-success');
						$('#bidalert').html("");
						$('#bidalert').addClass('alert-danger');
						$('#bidalert').html("<strong>Failed!</strong> Something went wrong. PLease try again!");
						$('#bidalert').css("display","block");
					}
				}
			});
