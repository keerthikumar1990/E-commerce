...
<!-- For google maps -->
<script type="text/javascript" src="http://maps.googleapis.com/maps/api/js?key=AIzaSyCwl-IEPmLd7TKv5K5lS-rR5p9qctbay1g"></script>
                ...
                <table class="table table-striped table-bordered" style="width: 100%">
                  ...
                  <tbody>
                    <tr ng-repeat="data in filtered = (list | filter:search | orderBy : predicate :reverse) | startFrom:(currentPage-1)*entryLimit | limitTo:entryLimit ">
                      ...
                      <td><input type="number" class="form-control" id="qty_{{data.bid_id}}"  style="width: 70px; display: inline-block" placeholder="Qty" required/><button class="btn btn-primary" id="addtocart_{{data.bid_id}}" style="margin-left: 10px">Add to cart</button></td>
                    </tr>
                  </tbody>
                </table>
                </form>
...
$(document).ready(function(){
	...
	$('body').on('click','[id^=addtocart]',function(){
		...
		if($('#qty_'+bidid).val() == ""){
	    	...
	    	qty = $('#qty_'+bidid).val();
	    	if(checkNumber(qty) != "valid"){
	    		$('#qty_'+bidid).tooltip('hide')
	            .attr('data-original-title', checkNumber(qty))
	            .tooltip('fixTitle')
	            .tooltip('show');
	    		return false;
	    	}
	    	$.ajax({
				...
				success : function(responseText) {console.log(responseText);
					if(responseText == 'inserted'){
						...
						$('#addtocartalert').html("<strong>Success!</strong> Item has been added to the cart successfully.");
						$('#addtocartalert').css("display","block");		
					}else if(responseText == 'updated'){
						$('#addtocartalert').removeClass('alert-danger');
						$('#addtocartalert').html("");
						$('#addtocartalert').addClass('alert-success');
						$('#addtocartalert').html("<strong>Success!</strong> You have already added this item to cart. Quantity has been updated successfully.");
						$('#addtocartalert').css("display","block");		
					}
					else{
						...
						$('#addtocartalert').html("<strong>Failed!</strong> Something went wrong. PLease try again!");
						$('#addtocartalert').css("display","block");
					}
				}
			});
